...
function doneOrNot(board){
  ...
  for (let i = 0; i < board.length; i++){
    let testRow= [...board[i]];
    if ( testRow.sort( (a,b) => a-b).join('') !== validRow ) return "Try again!";
  ...
  for (let i = 0; i < board.length; i++){
   ...
   if ( testColumn.sort( (a,b) => a-b).join('') !== validRow ) return "Try again!";  
  }
  ...
  for (let i = 0; i < board.length; i+=3) {
    ...
    for (let j = 0; j < board.length; j++){
      ...
      if (testCell.length === 9) { //ставим ограничение на 9 значений, так как при 9 значениях получаем наш квадрат.
        if ( testCell.sort((a,b)=>a-b).join('') !== validRow ) return "Try again!";
